...
export async function scanStudio(options: StudioScanOptions = {}): Promise<SpoofAsset[]> {
  ...
  try {
    ...
    logIsm('info', serviceText('studioScanDone', { count: assets.length }), true);
    return assets;
  } catch (error) {
...
export async function activateProfile(accountId: string, groupId: string | null = null) {
  ...
  if (!account) throw new Error(serviceText('profileNotFound', { id: accountId }));
    ...
    if (audioCount > 0) {
      ...
      if (quota && audioCount > quota.remaining) {
        return {
          ok: false,
          reason: 'quota',
          message: serviceText('quota', { count: audioCount, remaining: quota.remaining }),
  ...
  if (!downloadOnly && isTauriRuntime()) {
    try {
      ...
      if (
        ...
        warnings.push(
          serviceText('apiKeyOwnerMismatch', { owner: owner.ownerUserId, profile: target.userId }),
        );
      }
    } catch (e) {
      warnings.push(serviceText('apiKeyPrecheckFailed', { error: String(e) }));
    }
  }
...
export async function cancelJob(): Promise<boolean> {
  ...
  if (ok) log(`[WARN] ${serviceText('cancelRequested')}`);
  return ok;
}
...
export async function discoverPlaceIds(assetIds?: string[], timeoutSecs = 60) {
  ...
  const { cookie } = getActiveTarget();
  if (cookie.length < 50) throw new Error(serviceText('needCookieProfile'));
  const ids = assetIds?.length ? assetIds : selectedAssetIds();
  if (ids.length === 0) throw new Error(serviceText('nothingToDiscover'));
